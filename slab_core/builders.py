...
def canonicalize_periodic_positions(atoms):
    """Move periodic images into the canonical [0, 1) fractional cell.

    Only lattice-vector translations are applied, so distances, constraints,
    composition, and the physical periodic structure remain unchanged.
    """
    scaled = np.asarray(atoms.get_scaled_positions(wrap=False), dtype=float)
    scaled = np.mod(scaled, 1.0)
    scaled[np.isclose(scaled, 1.0, atol=1e-10)] = 0.0
    scaled[np.isclose(scaled, 0.0, atol=1e-12)] = 0.0
...
def build_standard_surface(route, atomic_layers, vacuum_total, supercell):
    ...
    canonicalize_periodic_positions(slab)
...
def build_general_terminations(
    ...
    for index, (pmg_slab, symmetrized) in enumerate(slabs):
        ...
        canonicalize_periodic_positions(ase_slab)
        metadata = {
            "route": "general",
            "hkl": list(route["hkl"]),
            "space_group_number": route["space_group_number"],
